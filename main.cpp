...
vector<vector<double> > readMatrix(const string& filename, int& n) {
    ifstream fin(filename.c_str());  // .c_str() для старых компиляторов
    if (!fin.is_open()) {
...
void writeMatrix(const string& filename, const vector<vector<double> >& mat, double elapsed_ms = -1.0) {
    ofstream fout(filename.c_str());  // .c_str() для старых компиляторов
    if (!fout.is_open()) {
...
pair<int, int> getRowRange(int rank, int size, int n) {
    ...
    return make_pair(start_row, local_rows);  // make_pair вместо { }
}
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        ...
        int n1, n2;
        vector<vector<double> > A_2d = readMatrix(filename_a, n1);  // auto → явный тип
        vector<vector<double> > B_2d = readMatrix(filename_b, n2);
    ...
    // auto [start_row, local_rows] → pair + .first/.second
    ...
    for (int p = 0; p < size; ++p) {
        // auto [p_start, p_rows] → pair + .first/.second
        pair<int, int> p_range = getRowRange(p, size, n);
        int p_start = p_range.first;
        int p_rows = p_range.second;
